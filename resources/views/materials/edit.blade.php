...
<div class="block block-rounded">
  <div class="block-header block-header-default">
    <h3 class="block-title">
      {{ trans('page.materials.edit') }}
    </h3>
  </div>
  <div class="block-content">

    <form action="{{ route('materials.update', $material->uuid) }}" method="POST" onsubmit="return disableSubmitButton()">
      @csrf
      @method('PATCH')
      ...
      <div class="row justify-content-center">
        <div class="col-md-6">
          ...
          <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $material->name) }}" class="form-control @error('name') is-invalid @enderror" onkeypress="return hanyaHuruf(event)">
            @error('name')
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="number" min="1" max="100" step="1" name="total" id="total" value="{{ old('total', $material->total) }}" class="form-control @error('total') is-invalid @enderror">
            @error('total')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <select name="mass" id="mass" class="js-select2 form-select @error('mass') is-invalid @enderror" data-placeholder="{{ trans('Pilih Massa Berat') }}" style="width: 100%;">
              ...
              <option value="{{ $item }}" @if (old('mass', $material->mass)==$item) selected @endif>{{ $item }}</option>
              @endforeach
            </select>
            @error('mass')
            <div class="invalid-feedback">{{ $message }}</div>
