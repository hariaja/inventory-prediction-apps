...
<div class="content content-full">
  <div class="content-heading">
    ...
    <nav class="breadcrumb push my-0">
      {{ Breadcrumbs::render('transactions.edit', $transaction) }}
    </nav>
  </div>
</div>
@endsection
        ...
        <div class="col-md-6">
          ...
          <div class="mb-4">
            ...
            <select name="product_id" id="product_id" class="js-select2 form-select @error('product_id') is-invalid @enderror" data-placeholder="{{ trans('Pilih Produk') }}" style="width: 100%;">
              ...
              <option value="{{ $item->id }}" data-uuid="{{ $item->uuid }}" @if (old('product_id', $transaction->product_id)==$item->id) selected @endif>{{ $item->name }}</option>
              @endforeach
            </select>
            @error('product_id')
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="number" min="1" max="25" step="1" name="quantity" id="quantity" value="{{ old('quantity', $transaction->quantity) }}" class="form-control @error('quantity') is-invalid @enderror" placeholder="{{ trans('Masukkan Jumlah') }}">
            @error('quantity')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
