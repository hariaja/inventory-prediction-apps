...
<div class="block block-rounded">
  <div class="block-header block-header-default">
    <h3 class="block-title">
      {{ trans('page.products.edit') }}
    </h3>
  </div>
  <div class="block-content">

    <form action="{{ route('products.update', $product->uuid) }}" method="POST" onsubmit="return disableSubmitButton()">
      @csrf
      @method('PATCH')
      ...
      <div class="row justify-content-center">
        <div class="col-md-6">
          ...
          <div class="mb-4">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" class="form-control @error('name') is-invalid @enderror" onkeypress="return hanyaHuruf(event)" placeholder="{{ trans('Masukan Nama Produk') }}">
            @error('name')
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="number" min="1" max="50" step="1" name="quantity_one_day" id="quantity_one_day" value="{{ old('quantity_one_day', $product->quantity_one_day) }}" class="form-control @error('quantity_one_day') is-invalid @enderror" placeholder="{{ trans('Masukkan Jumlah Stok Per Hari') }}">
          ...
          <div class="mb-4">
            ...
            <span class="text-danger">*</span>
            <input type="text" class="js-flatpickr form-control @error('produced_at') is-invalid @enderror" id="produced_at" name="produced_at" min="{{ date('Y-m-d') }}" placeholder="{{ trans('Pilih Tanggal Produksi') }}" value="{{ old('produced_at', $product->produced_at) }}">
            @error('produced_at')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
          ...
          <div class="mb-4">
            ...
            <textarea name="description" id="description" style="height: 130px" class="form-control @error('description') is-invalid @enderror" placeholder="{{ trans('Deskripsi Produk') }}">{{ old('description', $product->description) }}</textarea>
            @error('description')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          </div>
